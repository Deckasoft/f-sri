...
jest.mock('../src/models/DeliveryNote', () => {
  class MockDeliveryNote {
    ...
    static findOneAndUpdate = (...args: any[]) => deliveryNoteStaticMocks.findOneAndUpdate(...args);
    static findOneAndDelete = (...args: any[]) => deliveryNoteStaticMocks.findOneAndDelete(...args);
  }
...
describe('Delivery note routes', () => {
  ...
  describe('POST /api/v1/delivery-note/complete', () => {
    ...
    it('creates a complete delivery note and returns its XML', async () => {
      ...
      expect(crearGuiaRemisionCompletaMock).toHaveBeenCalledWith(guiaRemisionPayload, 'company-1');
    });
  ...
  describe('CRUD endpoints', () => {
    it('lists only the tenant’s own delivery notes', async () => {
      ...
      expect(deliveryNoteStaticMocks.find).toHaveBeenCalledWith({ empresa_emisora_id: 'company-1' });
    });

    it('returns 404 for a missing/cross-tenant delivery note', async () => {
      const res = await request(app).get('/api/v1/delivery-note/507f1f77bcf86cd799439011').set('X-API-Key', authHeader);

      expect(res.status).toBe(404);
    });

    it('returns a delivery note by id scoped to the tenant', async () => {
      deliveryNoteStaticMocks.findOne.mockResolvedValueOnce({ secuencial: '000000001' });
    ...
    it('returns the PDF info of a delivery note that belongs to the tenant', async () => {
      deliveryNoteStaticMocks.findOne.mockResolvedValueOnce({ _id: '507f1f77bcf86cd799439011' });
    ...
    it('404s the PDF lookup when the delivery note belongs to another tenant', async () => {
      const res = await request(app)
        .get('/api/v1/delivery-note/507f1f77bcf86cd799439011/pdf')
        .set('X-API-Key', authHeader);

      expect(res.status).toBe(404);
      expect(deliveryNotePDFStaticMocks.findOne).not.toHaveBeenCalled();
    });

    it('deletes a delivery note scoped to the tenant', async () => {
      deliveryNoteStaticMocks.findOneAndDelete.mockResolvedValueOnce({ _id: '507f1f77bcf86cd799439011' });
      ...
      expect(deliveryNoteStaticMocks.findOneAndDelete).toHaveBeenCalledWith({
